	...
	return (
		<div className="flex h-full flex-col border-l border-neutral-800">
			<div className="flex items-center gap-2 border-b border-neutral-800 px-3 py-2">
				...
				<span className="text-xs font-medium text-neutral-300">Debug Log</span>
				<span className="text-[10px] text-neutral-600">{entries.length} entries</span>
			</div>
			<div className="flex-1 overflow-y-auto text-xs">
				{entries.length === 0 && <div className="p-3 text-neutral-600">No requests yet</div>}
					...
					<div key={entry.id} className="border-b border-neutral-800/50">
						...
						>
							{expanded.has(entry.id) ? (
								<ChevronDown size={12} className="mt-0.5 shrink-0 text-neutral-500" />
							) : (
								<ChevronRight size={12} className="mt-0.5 shrink-0 text-neutral-500" />
							)}
							<div className="min-w-0 flex-1">
								<div className="flex items-center gap-2">
									<span
										className={`font-mono text-[10px] ${
											entry.direction === "inbound" ? "text-green-400" : "text-blue-400"
										}`}
									>
										{entry.direction === "inbound" ? "-> WEBHOOK" : "<- BOT API"}
									</span>
								...
								<div className="mt-0.5 truncate text-neutral-400 font-mono">
									{entry.method} {entry.url.split("/").slice(-2).join("/")}
								</div>
							</div>
						</button>
						{expanded.has(entry.id) && (
							<div className="bg-neutral-900 px-3 py-2">
								<div className="mb-1 text-[10px] text-neutral-500">Request Body</div>
