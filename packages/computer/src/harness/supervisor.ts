...
export const TaskSupervisorLive = Layer.scoped(
	...
	Effect.gen(function* () {
			...
			for (const task of [...activeTasks.values()]) {
				try {
					...
					if (task.consecutiveFailures >= MAX_HEARTBEAT_FAILURES) {
						const currentTask = await Effect.runPromise(
						...
						const completed = await Effect.runPromise(
							taskStore.completeTask({
								...
								payload: { reason: "heartbeat_failures" },
							}),
						)
						if (completed) {
		...
		async function finalizeTask(task: ActiveTask, exitCode: number) {
			...
			try {
				...
				const executionData = await collectTaskExecutionData({
					...
					provider,
					taskId: task.taskId,
					artifactRoot: runtimeData?.artifactRoot ?? provider.getArtifactRoot(task.taskId),
				})
				result = {
			...
			const completed = await Effect.runPromise(
				taskStore.completeTask({
					...
					exitCode,
					error: exitCode === 0 ? null : result.summary.slice(0, 4000),
					payload: {
						exitCode,
		...
		const recoverRunning = async () => {
			...
			try {
				running = await Effect.runPromise(taskStore.findRunningSandboxTasks())
			} catch (err) {
			...
			try {
				...
				const lostPreparing = await Effect.runPromise(
				...
				for (const row of lostPreparing) {
					const completed = await Effect.runPromise(
						...
						console.error(`[TaskSupervisor] recovery: failed to mark task ${row.id} lost:`, e)
						return false
					})
			...
			for (const task of running) {
				...
				try {
					...
					activeTasks.set(task.id, {
						...
						commandId: runtimeData.commandId,
					...
					const completed = await Effect.runPromise(
						taskStore.completeTask({
							...
							payload: { reason: "sandbox_recovery_failed" },
						}),
					)
					if (completed) {
						await appendTaskTraceTerminalEvent(traceStore, {
			...
			startTask: ({
				...
				Effect.gen(function* () {
					...
					const activeCount = yield* Effect.tryPromise({
						try: () => Effect.runPromise(taskStore.countActiveSandboxTasks(userId)),
						catch: (error) =>
							new SandboxError({
					...
					if (conversationId) {
						const platform = yield* Effect.tryPromise({
							try: () => Effect.runPromise(taskStore.getConversationPlatform(conversationId)),
							catch: (error) =>
								new SandboxError({
						...
						if (platform === "telegram") {
							const accountMetadata = yield* Effect.tryPromise({
								try: () => Effect.runPromise(taskStore.getTelegramAccountMetadata(userId)),
								catch: (error) =>
									new SandboxError({
							...
							try {
								...
								await Effect.runPromise(
									taskStore.completeTask({
										...
										summary: message,
										error: message,
										payload: { error: message, reason: "startup_failed" },
									}),
								)
								await deletePendingSession(sandbox, sessionId)
								throw cause
			...
			getTask: (taskId, userId, waitSeconds) =>
				Effect.gen(function* () {
					...
					if (
						...
						readSandboxRuntimeData(task)?.sessionId &&
						readSandboxRuntimeData(task)?.commandId
					) {
						...
						if (sandbox) {
							...
							const taskRef = task
							const sessionId = runtimeData.sessionId
							const commandId = runtimeData.commandId
			...
			probeTask: (taskId, userId) =>
				Effect.gen(function* () {
					const task = yield* Effect.tryPromise({
						try: () => Effect.runPromise(taskStore.getByIdAndUser(taskId, userId)),
						catch: (error) =>
							new SandboxError({
					...
					const updated = yield* Effect.tryPromise({
						try: () => Effect.runPromise(taskStore.getById(taskId)),
						catch: (error) =>
							new SandboxError({
			...
			getTaskArtifacts: (taskId, userId) =>
				Effect.gen(function* () {
					const executionData = yield* Effect.tryPromise({
						try: async () => {
							const task = await Effect.runPromise(taskStore.getByIdAndUser(taskId, userId))
							if (!task) return null
							...
							return await collectTaskExecutionData({
								...
								provider,
								taskId,
								artifactRoot: runtimeData.artifactRoot,
							})
						},
						catch: (error) =>
							new SandboxError({
