	...
	Effect.gen(function* () {
		...
		const upsertPendingAuthRequest = (
			...
			Effect.gen(function* () {
				...
				// Delete any existing pending row for this user+provider, then insert a new one.
				// This avoids needing a unique constraint on (userId, provider, status).
				yield* Effect.tryPromise({
					...
					catch: mapDatabaseError("failed_to_clear_stale_pending_auth_request"),
				})
			...
			query((database) =>
				database
					...
					.where(
						and(
							eq(schema.integrationAccounts.status, "pending"),
							sql`${schema.integrationAccounts.metadataJson}->>'expiresAt' <= ${new Date().toISOString()}`,
						),
					),
			).pipe(
				Effect.asVoid,
